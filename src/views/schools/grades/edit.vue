...
const Group = new Restlet('/data/terminal/group', '/data/terminal/group')
export default {
  name: 'GroupEdit',
  components: { },
  props: {
  ...
  watch: {
    editObject: function(newVal, oldVal) {
      ...
      if (newVal !== oldVal) {
        ...
        if (this.editForm.id) {
          this.dialogTitle = '编辑分组：' + this.editForm.name
      ...
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          if (updateData.length === 0) {
            ...
            if (this.editForm.id) {
              Group.update(this.editForm.id, updateData).then(res => {
              ...
              Group.create(this.editForm).then(res => {
                console.log('handleSave:>', res)
                this.$notify({
                  title: '成功',
                  message: '更新成功！',
